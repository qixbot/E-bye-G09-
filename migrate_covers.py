...
def migrate_covers():
    print("🔁 开始迁移封面图到 Cloudinary...")
    
    conn = psycopg2.connect(DATABASE_URL)
    cur = conn.cursor()
    
    cur.execute("""
        SELECT id, username, cover_blob 
        FROM users 
    ...
    for user in users:
        ...
        try:
            ...
            print(f"📤 上传 {username} 的封面图...")
            url = upload_image(blob_data, folder="e-bye/covers")
    ...
    print(f"\n📊 封面图迁移完成！成功: {success_count}, 失败: {fail_count}")
