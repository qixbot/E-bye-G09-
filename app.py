...
# Global flag to control database-dependent auto functions
DB_AVAILABLE = True  # Will be set to False if database connection fails
...
def login():
    if request.method == 'POST':
        ...
        if user and check_password_hash(user['password'], password):
            ...
            # Check freeze
            if user.get('is_frozen') == 1 and user.get('frozen_until'):
                now = datetime.now()
                expire_time = None
                try:
...
# ============================================================
# AUTO UNFREEZE EXPIRED - DISABLED (fix SSL issue)
# ============================================================
# @app.before_request
...
# ============================================================
# CHECK REMEMBER ME - WITH ERROR HANDLING
# ============================================================
@app.before_request
def check_remember_me():
    ...
    try:
        ...
        print(f"check_remember_me database error: {e}")
        # Don't block the page, just continue without auto-login
...
def home():
    ...
    try:
        ...
        print(f"Home page database error: {e}")
        flash('Unable to load products. Please try again later.', 'error')
        return render_template('home.html', username=session.get('username'), latest_products=[])
...
def upload_background():
    ...
    try:
        ...
        cur.close()
        db.close()
        return jsonify({'success': True, 'bg_value': bg_value})
    except Exception as e:
...
# ============================================================
# API ENDPOINTS - Purchases & Listings
# ============================================================
...
def accept_counter_offer(offer_id):
    ...
    try:
        ...
        print(f"Accept counter error: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500
...
def api_update_product_full(product_id):
    ...
    try:
        ...
        print(f"Update full product error: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500
...
def switch_to_admin():
    ...
    try:
        ...
        print(f"Switch to admin error: {e}")
        flash('Unable to switch to admin mode', 'error')
        return redirect(url_for('edit_profile'))
    ...
    if request.method == 'POST':
        ...
        if step == '1':
            ...
            try:
                ...
                print(f"Forgot password step1 error: {e}")
            ...
            try:
                ...
                print(f"Forgot password step2 error: {e}")
                flash('Database error. Please try again.', 'error')
                return render_template('forgot_password.html')
            ...
            try:
                ...
                print(f"Forgot password step3 error: {e}")
...
# ============================================================
# CHECK ADMIN REMEMBER ME - WITH ERROR HANDLING
# ============================================================
@app.before_request
def check_admin_remember_me():
    ...
    try:
        ...
        print(f"check_admin_remember_me database error: {e}")
...
def reject_product(pid):
    ...
    try:
        ...
        flash("Failed to reject product", "error")
...
def freeze_7day(user_id):
    ...
    try:
        ...
        print(f"Freeze user error: {e}")
        flash("Failed to freeze user", "error")
...
def unfreeze_user(user_id):
    ...
    try:
        ...
        print(f"Unfreeze user error: {e}")
        flash("Failed to unfreeze user", "error")
...
def handle_report(report_id, action):
    ...
    try:
        ...
        print(f"Handle report error: {e}")
        return jsonify({'success': False}), 500

# ============================================================
# Keting's Route - Chat Routes
# ============================================================
...
def chat_send():
    ...
    try:
        ...
        ''', (session['user_id'], int(receiver_id), int(product_id) if product_id else None))
        db.commit()
        cur.close()
        db.close()
        return jsonify({'success': True})
...
def chat_send_images():
    ...
    try:
        ...
        print(f"Chat send images error: {e}")
        return jsonify({'success': False}), 500
...
def chat_send_image():
    ...
    try:
        ...
        print(f"Chat send image error: {e}")
        return jsonify({'success': False}), 500
...
def chat_page(other_user_id, product_id=None):
    ...
    try:
        ...
        flash("Unable to load chat", "error")
        return redirect(url_for('home'))
...
def chat_list():
    ...
    try:
        ...
        return render_template('user_chatlist.html', chats=[], unread_notifications=0, unread_reviews=0, unread_announcements=0)
...
def upload_product():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            print(f"Upload product error: {e}")
            flash("Failed to upload product. Please try again.", "error")
            return render_template('upload.html')
...
def clear_products():
    try:
        ...
        print(f"Clear products error: {e}")
        return "Error clearing products."
...
# ============================================================
# Xingru's Route - User Profile (temporary)
# ============================================================
